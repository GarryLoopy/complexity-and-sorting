...
public class ComplexityAndSorting {
    ...
    public static void main(String[] args) {
        ...
        else
        {
            ...
            {
                case "v" -> RunSortForShapes(new ShapeVolumeCompare());
                case "a" -> RunSortForShapes(new ShapeBaseAreaCompare());
                
            }
        }
        
        for (Shape shape : shapeHelper.GetShapes())
        {
        ...
        private static void RunSortForShapes()
    {
        ...
        {
            case "b" -> Sort.Bubble(data);
            case "s" -> Sort.Selection(data);
            case "i" -> Sort.Insertion(data);
            case "m" -> Sort.Merge(data);
            case "q" -> Sort.Quick(data, 0, amountOfData - 1);
            case "z" -> Sort.Heap(data);
        }
    }
    ...
    private static void RunSortForShapes(Comparator<Shape> comparator)
    {
        ...
        switch (userInput.get(sortArgument))
        {
            case "b" -> Sort.Bubble(data, comparator);
            case "s" -> Sort.Selection(data, comparator);
            case "i" -> Sort.Insertion(data, comparator);
            case "m" -> Sort.Merge(data, comparator);
            case "q" -> Sort.Quick(data, 0, amountOfData - 1, comparator);
            case "z" -> Sort.Heap(data, comparator);
        }
    }
